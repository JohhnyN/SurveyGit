...
from ..mixin import ContextTitleMixin
from ..views import SurveyListView
from ..forms import BaseSurveyForm
...
class AdminCrateSurveyView(ContextTitleMixin, CreateView):
    model = Survey
    template_name = 'djf/admins/form.html'
    fields = [
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            survey = form.save()
            self.success_url = reverse("djf:admin_forms_survey", args=[survey.slug])
...
class AdminEditSurveyView(ContextTitleMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse("djf:admin_forms_survey", args=[survey.slug])


@method_decorator(staff_member_required, name='dispatch')
class AdminSurveyListView(SurveyListView):
    template_name = 'djf/admins/survey_list.html'


@method_decorator(staff_member_required, name='dispatch')
class AdminSurveyFormView(ContextTitleMixin, FormMixin, DetailView):
    model = Survey
    template_name = 'djf/admins/form_preview.html'
...
class AdminDeleteSurveyView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect("djf:admin_survey")


@method_decorator(staff_member_required, name='dispatch')
class AdminCreateQuestionView(ContextTitleMixin, CreateView):
    model = Question
    template_name = 'djf/admins/question_form.html'
    success_url = reverse_lazy("djf:")
    ...
    def get_success_url(self):
        return reverse("djf:admin_forms_survey", args=[self.survey.slug])


@method_decorator(staff_member_required, name='dispatch')
class AdminCreateHeadingView(ContextTitleMixin, CreateView):
    model = Question
    template_name = 'djf/admins/form.html'
    success_url = reverse_lazy("djf:")
    fields = ['label']
    title_page = _("Add Heading")
    survey = None
...
class AdminDeleteQuestionView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect("djf:admin_forms_survey", slug=self.survey.slug)
...
class SummaryResponseSurveyView(ContextTitleMixin, DetailView):
    model = Survey
    template_name = "djf/admins/summary.html"
