...
from .models import Survey, UserAnswer
from .forms import CreateSurveyForm, EditSurveyForm
from .mixin import ContextTitleMixin
from . import app_settings
from .utils import NewPaginator


class SurveyListView(ContextTitleMixin, ListView):
    model = Survey
    title_page = 'Список опросов'
...
class SurveyFormView(FormMixin, DetailView):
    template_name = 'djf/form.html'
    success_url = reverse_lazy("djf:index")
...
class DeleteSurveyAnswerView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect("djf:detail", slug=user_answer.survey.slug)


class DetailSurveyView(ContextTitleMixin, DetailView):
    model = Survey
    template_name = "djf/answer_list.html"
...
class DetailResultSurveyView(ContextTitleMixin, DetailView):
    ...
    template_name = "djf/detail_result.html"
    model = UserAnswer
...
def share_link(request, slug):
    ...
    if request.user.is_authenticated:
        ...
        if user_answer:
            return redirect(reverse_lazy("djf:edit", kwargs={'pk': user_answer.id}))
    return redirect(reverse_lazy("djf:create", kwargs={'slug': survey.slug}))